...
def update(current):
    ...
    First.set(mylist[current].getfirstname())
    Last.set(mylist[current].getlastname())
    Phone.set(mylist[current].getphone())
    Address.set(mylist[current].getaddress())
    City.set(mylist[current].getcity())
    State.set(mylist[current].getstate())
    Zip.set(mylist[current].getzip())
    return
...
def fileOpen(fileName):
    ...
    for line in database:
        ...
        alist.append(person(items[0], items[1], items[2], items[3], items[4], items[5], items[6]))
    database.close()
    return alist
